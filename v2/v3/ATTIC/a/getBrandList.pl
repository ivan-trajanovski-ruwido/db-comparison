...
$sql .= " AND from_table like 'model_%'";
	...
	if ($param->{key} eq "d898981e9d82" || #KLUDGE: bouygues - no limit for brands
	    $param->{key} eq "9385807aba71" || # ruwido
	    $param->{key} eq "3da1344683b4") { #KLUDGE: telus - no limit for brands
		if ($param->{limit}) {
		...
		$sql .= " LIMIT " . $db->cleanLimit($param->{limit}); # if ($param->{limit} ne "-1");
	}
