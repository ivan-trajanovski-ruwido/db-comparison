...
sub select_code
{
	...
	print $q->Tr($q->th(["Database", "Project", "Revision", "Date", "CodeManual", "BrandManual"]));
	...
	$sql = "SELECT * FROM code_meta WHERE project_name NOT LIKE '%text%' ORDER BY project_id, revision_number, project_name, revision_name" if ($param->{order} eq "db");
...
#			push @csv1,  "<a href=\"/internal/".$param->{key}."/v2/codeManual/".$entry->{project_id}."/".$entry->{revision_id}."/$entry2->{id}?output=csv\">CSV:$entry2->{name}</a>";
		...
		my $new_id = sprintf("%04d%02d", $entry->{project_id}, $entry->{revision_number});

		next if ($codes eq "");
		print $q->Tr($q->td([$new_id, $entry->{project_name}, $entry->{revision_name}, $entry->{date}, $codes, $brands, join(" ", @csv1)]));
	}
	$sth->finish();
