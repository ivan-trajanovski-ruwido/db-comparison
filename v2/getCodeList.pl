...
sub select_brandcode
{
	...
	if (defined $param->{brand_id} && $param->{brand_id} ne '') {
		$sql .= $db->bind("AND brand_id = ?", $param->{brand_id}, \@bind_values) if ($param->{brand_id});
	} elsif (defined $param->{brand_name} && $param->{brand_name} ne '') {
		$sql .= $db->bind"AND brand_id IN (SELECT id FROM brand WHERE name sREGEXP= ?)", $param->{brand_name}, \@bind_values) if ($param->{brand_name});
        }
	$sql .= $db->bind("AND 1<<(type_id-1) & ?", $db->{__types},  \@bind_values) if ($db->{__types});
	$sql .= $db->bind("AND project_id = ?", $param->{project}, \@bind_values);
	$sql .= $db->bind("AND revision_id = ?", $param->{revision}, \@bind_values);
...
#		$sql .= " LEFT JOIN ruwido_rc.weight_oem USING (rc_oem_id)";
		...
		if (defined $param->{brand_id}) {
			$sql .= $db->bind("AND rc_info.brand_id = ?", $param->{brand_id}, \@bind_values);
		} elsif(defined $param->{brand_name}) {
			$sql .= $db->bind("AND rc_info.brand_id IN (SELECT id FROM brand WHERE name = ?)", $param->{brand_name}, \@bind_values);
		}
		...
		$sql_oem_ids .= $db->bind("AND mv_union.type_set & ?", $db->{__types}, \@bind_values) if ($param->{__types});
                $sql_oem_ids .= $db->bind("AND FIND_IN_SET(?, mv_union.type_set)", $db->cleanTypeId($param->{type_name}), \@bind_values) if ($param->{type_name});

		$sql_oem_ids .= $db->bind("AND mv_union.brand_id = ?", $param->{brand_id}, \@bind_values) if (defined $param->{brand_id} && $param->{brand_id} ne '');
		$sql_oem_ids .= $db->bind("AND YEAR(mv_union.date) >= ?", $param->{year}, \@bind_values) if (defined $param->{year} && $param->{year} ne '');
		if (defined $param->{model_id} && $param->{model_id} ne '') {
			$sql_oem_ids .= $db->bind("AND mv_union.id = ?", $param->{model_id}, \@bind_values);
		} elsif (defined $param->{model_name} && $param->{model_name} ne '') {
			$sql_oem_ids .= $db->bind("AND mv_union.name REGEXP ?", $db->createSearch($param->{model_name}), \@bind_values);
		}
