...
# only check for oem_id
	...
	### compression step 1: simple pre-compress
	...
	if (defined $param->{compress} && $param->{compress} <= 0) {
	...
	} else { # generate fingerprints and derive only uniqe rc_oem_ids
#		$sql_rc_oem_num_weight = "SELECT rc_oem_id, type_layer, COUNT(rc_oem_id) AS rc_oem_num, SUM(weight) AS weight FROM (SELECT rc_oem_id, type_layer, $weight_select, GROUP_CONCAT(' ', func_id, ':', descr) AS fingerprint FROM oem_to_signal";
		$sql_rc_oem_num_weight = "SELECT rc_oem_id, type_layer, COUNT(rc_oem_id) AS rc_oem_num, SUM(weight) AS weight FROM (SELECT rc_oem_id, type_layer, $weight_select, GROUP_CONCAT(' ', func_id, ':', data) AS fingerprint FROM oem_to_signal";
		$sql_rc_oem_num_weight .= " WHERE $sql_rc_oem_ids";
		$sql_rc_oem_num_weight .= " AND func_id IN ($func_ids)";
		if (defined $param->{type_id}) {
			$sql_rc_oem_num_weight .= $db->bind_and("type_id = ?", $db->cleanTypeId($param->{type_id}), \@bind_values);
			$sql_rc_oem_num_weight .= " GROUP BY rc_oem_id, type_id, type_layer ORDER BY rc_oem_id DESC) AS tbl1 GROUP BY fingerprint";	# FIXME: order by clause is just there to sort the wrong way, since the oldest samsung does not have and input key, and so the majority of samsungs would have no input keys
		} else {
			$sql_rc_oem_num_weight .= " GROUP BY rc_oem_id, type_layer ORDER BY rc_oem_id DESC) AS tbl1 GROUP BY fingerprint";	# FIXME: order by clause is just there to sort the wrong way, since the oldest samsung does not have and input key, and so the majority of samsungs would have no input keys
		}
	}
...
my $prefix = undef;
		$virtual_remote = [];
if ($param->{compress} == 3 && defined $param->{debug}) {
		foreach my $btn_id (sort { $a <=> $b } keys $items{$oem_id}) {
			next if !$self->isint($btn_id);

			my $_prefix = join "", unpack ('H2' x length($items{$oem_id}{$btn_id}{content}), $items{$oem_id}{$btn_id}{content});
#			substr($_prefix,25,1)= "X";
			substr($_prefix,17,1)= "X";

			if (! defined $prefix) {
				$prefix = $_prefix;
			} else {
				$prefix = _lcp($prefix, $_prefix);
			}
		}
}
...
#				substr($items{$oem_id}{$btn_id}{content},25,1)= "X";
				substr($items{$oem_id}{$btn_id}{content},17,1)= "X";
				$items{$oem_id}{$btn_id}{content} =~ s/\Q$prefix//;
} else {
				$items{$oem_id}{$btn_id}{content} = join ",", unpack ('H2' x length($items{$oem_id}{$btn_id}{content}), $items{$oem_id}{$btn_id}{content});
}
			}
...
if ($param->{compress} == 3 && defined $param->{debug}) {
			push @$items_arr, {id => $oem_id, weight => $weight, num => $num, signal => $virtual_remote, prefix => $prefix};
} else {
