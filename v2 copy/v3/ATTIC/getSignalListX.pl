...
#$param{oem_id_min} = '13000' if (!defined $param{oem_id_min} && !defined $param{oem_id});
...
if ($db->checkAccess(\%param)) {
	...
	if ($param{key} eq $signalList->{key_master}) {
		$db->print($q, $signalList->{total}, {host => "new", sql => $signalList->{sql}, database => $db->{db}, remotes => {total => $signalList->{total}, num => $signalList->{num}, remote => $signalList->{remote}}}, {-vary => "Accept",
...
sub output_to_base64
{
	...
	foreach my $id (keys $signalList->{remote}) {
		...
		foreach my $foo (keys $signalList->{remote}[$id]->{signal}) {
			...
			my $fkt = $signalList->{remote}[$id]->{signal}[$foo]->{id};
			my $len_sig = scalar @arr;
			my $sig = join "", @arr;
